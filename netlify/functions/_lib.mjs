...
export async function getJSON(storeName,key,fallback=null){
  const v=await store(storeName).get(key,{type:'json',consistency:'strong'});
  return v ?? fallback;
}
...
export function readSession(req){
  ...
  const m=raw.match(/(?:^|;\s*)sapucaia_session=([^;]+)/);
  if(!m)return null;
...
export async function getAdminCredentials(){
  return getJSON('sapucaia-config',ADMIN_KEY,null);
}

export async function setAdminCredentials(v){
  return putJSON('sapucaia-config',ADMIN_KEY,v);
}
...
export function verifyPassword(password,stored){
  ...
  try{
    const a=Buffer.from(
      crypto.scryptSync(String(password),stored.salt,64).toString('hex'),
      'hex'
    );
...
const DEFAULT_SETTINGS={
  ...
  backgroundImage:'https://cdn.discordapp.com/attachments/1419475351989391503/1548871162723967026/sapucaia_rio_wallpaper-1.gif?ex=6aa8a22d&is=6aa750ad&hm=b9bcc332a0cc3bb714bf13debfe339f5d068aed3e105a9386306a96d2598f38d&',
  ...
  // O Handle será lido da variável do Netlify
  infinitePayHandle:'',
